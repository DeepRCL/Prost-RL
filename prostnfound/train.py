...
def main(cfg):
    ...
    wandb.init(
        ...
        **cfg.get("wandb", {}),
        project="prostate_cancer_detection",
        name=cfg.name,
    )
    _tmpdir = mkdtemp()
...
def show_example(args, data):
    ...
    needle_mask = T.Resize(args.image_size, interpolation=Image.Resampling.NEAREST)(
        needle_mask
    )
    prostate_mask = T.Resize(args.image_size, interpolation=Image.Resampling.NEAREST)(
        prostate_mask
    )
    ...
    # prediction
    pred = T.Resize(args.image_size)(pred)
